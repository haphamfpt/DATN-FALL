...
export const getBlogs = asyncHandler(async (req, res) => {
  ...
  const blogs = await Blog.find(query)
    .select('title content views isPublished slug excerpt coverImage readTime createdAt tags author')
    .sort({ createdAt: -1 })
    .skip(skip)
    .limit(limit);
...
export const createBlog = asyncHandler(async (req, res) => {
  ...
  if (!title || !excerpt || !content || !coverImage) {
    res.status(400);
    throw new Error('Vui lòng điền đầy đủ tiêu đề, tóm tắt, nội dung và ảnh bìa');
  }
