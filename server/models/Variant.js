...
variantSchema.pre('save', async function(next) {
  if (!this.sku) {
    ...
    if (product && color && size) {
      const shortName = product.name.split(' ').slice(0, 2).join('-').toUpperCase();
      this.sku = `${shortName}-${color.attribute_color_code.replace('#', '')}-${size.attribute_size_name.replace('Size ', '')}`;
    } else {
